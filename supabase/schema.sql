...
drop table if exists public.replanning_events cascade;
drop table if exists public.calendar_connections cascade;
drop table if exists public.usage_counters cascade;
drop table if exists public.intent_model_routes cascade;
...
drop table if exists public.chat_request_replays cascade;
...
create table public.profiles (
  ...
  replanning_mode text not null default 'semi_automatic' check (replanning_mode in ('suggest_only', 'semi_automatic', 'automatic')),
  google_calendar_enabled boolean not null default false,
  outlook_calendar_enabled boolean not null default false,
  internal_risk_detection_enabled boolean not null default true,
...
create table public.chat_request_replays (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references public.profiles (id) on delete cascade,
...
create table public.calendar_events (
  ...
  source_provider text check (source_provider in ('google', 'outlook', 'manual')),
...
create table public.replanning_events (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references public.profiles (id) on delete cascade,
...
create table public.intent_model_routes (
  plan_code text not null references public.plan_definitions (code) on delete cascade,
  channel text not null check (channel in ('text', 'voice')),
...
create index idx_chat_request_replays_user_created_at on public.chat_request_replays (user_id, created_at desc);
...
create index idx_calendar_events_user_provider on public.calendar_events (user_id, source_provider, start_minute);
create index idx_calendar_connections_user_provider on public.calendar_connections (user_id, provider);
create index idx_replanning_events_user_created_at on public.replanning_events (user_id, created_at desc);
create index idx_replanning_events_user_hash on public.replanning_events (user_id, trigger_hash, created_at desc);
