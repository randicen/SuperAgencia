                            ...
                            <div className="flex justify-between items-start mb-2 pl-1">
                                ...
                                <div className="flex gap-1">
                                    ...
                                    <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-md ${
                                        p.priority === Priority.ASAP ? 'bg-purple-600 text-white' :
...
const CalendarMode = ({ projects, onEditTask }: { projects: Project[], onEditTask: (p: Project) => void }) => {
    ...
    const isProjectActiveOnDay = (p: Project, day: number) => {
        ...
        const startParts = p.startDate.split('-').map(Number);
        const startDate = new Date(startParts[0], startParts[1] - 1, startParts[2], 0, 0, 0).getTime();
        const endParts = p.endDate.split('-').map(Number);
        const endDate = new Date(endParts[0], endParts[1] - 1, endParts[2], 23, 59, 59, 999).getTime();
        
        return startDate <= dayEndTimestamp && endDate >= dayStartTimestamp;
    };
                        ...
                        <div key={day} className={`h-32 bg-white border border-slate-100 rounded-xl p-2 relative hover:border-blue-300 transition-colors overflow-hidden group ${isToday ? 'ring-2 ring-blue-500/20' : ''}`}>
                            ...
                            <div className="space-y-1 overflow-y-auto max-h-[90px] custom-scrollbar">
                                {activeProjects.map(p => (
                                    <div key={p.id} onClick={() => onEditTask(p)} className={`text-[8px] px-2 py-1 rounded-md font-bold uppercase truncate cursor-pointer hover:opacity-80 transition-opacity border-l-2 shadow-sm ${
                                            p.priority === Priority.ASAP ? 'bg-purple-50 text-purple-700 border-purple-500' :
...
const GanttView: React.FC<GanttViewProps> = ({ 
  ...
  const getPosition = (startDateStr: string, endDateStr: string) => {
    // Parseamos manualmente los componentes para crear una fecha Local exacta a las 00:00:00.
