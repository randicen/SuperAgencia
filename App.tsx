...
const App: React.FC = () => {
  ...
  // GOOGLE CALENDAR SENSOR SYNC
  useEffect(() => {
    if (!rules.gcalIcalUrl) return;

    const fetchGCal = async () => {
      try {
        setSyncStatus('syncing');
        // Using a CORS proxy as iCal is often blocked from direct browser access
        // We use allorigins.win or similar as it's a standard tool for simple client-side proxies
        const response = await fetch(`https://api.allorigins.win/get?url=${encodeURIComponent(rules.gcalIcalUrl)}`);
        if (!response.ok) throw new Error('Network error');
        
        const data = await response.json();
        const icalData = data.contents;

        // Simple regex-based ical parser for minimal weight (VEVENT parsing)
        const events: any[] = [];
        ...
        const parseIcalDate = (str: string) => {
          if (!str) return null;
          // Format: 20260320T190000Z or 20260320
          const clean = str.replace(/[:;]/g, '').split('VALUE=DATE')[0].trim();
        ...
        const now = new Date();
        const limit = new Date(); limit.setDate(now.getDate() + 14); // Next 14 days only for performance

        while ((match = eventRegex.exec(icalData)) !== null) {
          const content = match[1];
          const summary = (content.match(/SUMMARY:(.*)/)?.[1] || 'Evento GCal').trim();
          const dtStartRaw = content.match(/DTSTART(?:;[^:]*)?:(.*)/)?.[1];
          const dtEndRaw = content.match(/DTEND(?:;[^:]*)?:(.*)/)?.[1];
          ...
          if (dtStartRaw && dtEndRaw) {
            ...
            if (start && end) {
              ...
              if (startDate >= now && startDate <= limit) {
                events.push({
                  id: `gcal-${Math.random()}`,
                  nombre: summary,
                  startDate: start,
        ...
        console.log(`[GCal Sensor] Imported ${events.length} upcoming events`);
        
        // Dispatch to SpacesContext (it handles trigger scheduling)
        window.dispatchEvent(new CustomEvent('coo_gcal_events_updated', { detail: events }));
        setSyncStatus('synced');
      } catch (err) {
    ...
    fetchGCal();
    const gcalInterval = setInterval(fetchGCal, 300000); // Sync every 5 mins
